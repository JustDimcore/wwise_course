...
public class vThirdPersonCamera : MonoBehaviour
{
    ...
    void CameraMovement()
    {
        ...
        var targetPos = new Vector3(position.x, position.y + offSetPlayerPivot, position.z);
        currentTargetPos = targetPos;
        var desiredPlayerPos = targetPos + new Vector3(0, height, 0);
        var currentPlayerPos = currentTargetPos + new Vector3(0, currentHeight, 0);

        ClipPlanePoints planePoints = _camera.NearClipPlanePoints(currentPlayerPos + (camDir * (distance)), clipPlaneMargin);
        ClipPlanePoints oldPoints = _camera.NearClipPlanePoints(desiredPlayerPos + (camDir * distance), clipPlaneMargin);

        //Check if Height is not blocked 
        if (Physics.SphereCast(targetPos, checkHeightRadius, Vector3.up, out var hitInfo, cullingHeight + 0.2f, cullingLayer))
        {
        ...
        if (CullingRayCast(desiredPlayerPos, oldPoints, out hitInfo, distance + 0.2f, cullingLayer, Color.blue))
        {
            distance = hitInfo.distance - 0.2f;
            if (distance < defaultDistance)
            {
                var hitDistance = hitInfo.distance;
                hitDistance -= cullingMinDist;
                hitDistance /= cullingMinDist;
                currentHeight = Mathf.Lerp(cullingHeight, height, Mathf.Clamp(hitDistance, 0.0f, 1.0f));
                currentPlayerPos = currentTargetPos + new Vector3(0, currentHeight, 0);
            }
        }
        else
        {
        ...
        if (CullingRayCast(currentPlayerPos, planePoints, out hitInfo, distance, cullingLayer, Color.cyan)) 
            distance = Mathf.Clamp(cullingDistance, 0.0f, defaultDistance);
        var lookTargetPoint = currentPlayerPos + targetLookAt.forward * 2f;
        var right = targetLookAt.right;
        lookTargetPoint += (right * Vector3.Dot(camDir * (distance), right));
        targetLookAt.position = currentPlayerPos;

        Quaternion newRot = Quaternion.Euler(mouseY, mouseX, 0);
        targetLookAt.rotation = Quaternion.Slerp(targetLookAt.rotation, newRot, smoothCameraRotation * Time.deltaTime);
        var transform1 = transform;
        transform1.position = currentPlayerPos + (camDir * (distance));
        var rotation = Quaternion.LookRotation((lookTargetPoint) - transform1.position);
