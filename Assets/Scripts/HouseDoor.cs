...
public class HouseDoor : ClickableObject
{
    ...
    [SerializeField] private AK.Wwise.Event houseDoorOpened; // Івент для відкриття
    [SerializeField] private AK.Wwise.Event houseDoorClosed; // Івент для закриття
    [SerializeField] private AK.Wwise.Event houseDoorMovingStart; // Івент для початку руху дверей
    ...
    public override void OnClick()
    {
        ...
        {
            houseDoorOpened?.Post(gameObject); // Виклик івенту відкриття при кліці, якщо двері повністю закриті (Rotation Y дорівнює 0)
        }

        houseDoorMovingStart?.Post(gameObject); // Виклик івенту руху дверей при кожному кліці
    ...
    private IEnumerator OpenCloseDoor()
    {
        ...
        if (!_isOpening)
            houseDoorClosed?.Post(gameObject); // Виклик івенту закриття після завершення руху дверей
    }
}